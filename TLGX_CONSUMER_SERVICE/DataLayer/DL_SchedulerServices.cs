    ...
    public class DL_SchedulerServices : IDisposable
    {
        ...
        public List<DC_UnprocessedData> getTaskGeneratorFiles()
        {
            ...
            try
            {
                ...
                //sb.Append(" from Supplier_Schedule sch with(nolock) inner join Supplier spr with(nolock) on sch.Supplier_ID = spr.Supplier_Id left join Supplier_Scheduled_Task tsk        ");
                //sb.Append(" with(nolock) on sch.SupplierScheduleID = tsk.Schedule_Id where spr.StatusCode = 'ACTIVE' and sch.Entity is not null and len(CronExpression) > 0             ");
                //sb.Append(" ) select SupplierScheduleID, ScheduleDate, CronExpression, Api_Call_Log_Id, ");
                //sb.Append(" ISXMLSupplier, Status from CTE where rowNUm = 1 And Convert(Datetime,ScheduleDate) <= GETDATE();; ");

                //sb.Append(" select SupplierScheduleID,tsk.Status Status,isnull(tsk.Schedule_Datetime,getdate()) As ScheduleDate, sch.CronExpression,Api_Call_Log_Id As Api_Call_Log_Id, ");
                //sb.Append(" isnull(sch.ISXMLSupplier,1) ISXMLSupplier from Supplier_Schedule sch with(nolock) ");
                //sb.Append(" inner join Supplier spr with(nolock) on sch.Supplier_ID = spr.Supplier_Id ");
                //sb.Append(" left join Supplier_Scheduled_Task tsk  with(nolock) on sch.SupplierScheduleID = tsk.Schedule_Id and tsk.Status <> 'Completed' ");
                //sb.Append(" where spr.StatusCode = 'ACTIVE' and sch.Entity is not null and len(CronExpression) > 0 and convert(date, isnull(tsk.Schedule_Datetime, GETDATE())) <= Convert(date, getdate()) ;");
