                    ...
                    if (obj.SupplierImportFile_Id != null)
                    {
                        ...
                        if (stat != null)
                        {
                            ...
                            else if (resstat[0].Entity.Trim().ToUpper() == "CITY")
                            {
                                ...
                                {
                                    stat.FinalStatus = (from a in context.SupplierImportFileDetails where a.SupplierImportFile_Id == obj.SupplierImportFile_Id select a.STATUS).FirstOrDefault();
                                    stat.Unmapped = (stat.Unmapped ?? 0) + stat.Unmapped + getcount.Where(g => g.Status.Trim().ToUpper() == "UNMAPPED").Select(a => a.count).FirstOrDefault();
                                    stat.Mapped = (stat.Mapped ?? 0) + stat.Mapped + getcount.Where(g => g.Status.Trim().ToUpper() == "MAPPED" || g.Status.Trim().ToUpper() == "REVIEW").Select(a => a.count).FirstOrDefault();
                                }
                                else
                                {
                ...
                using (ConsumerEntities context = new ConsumerEntities())
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(mySupplier))
                    {
                        ...
                        foreach (DataContracts.STG.DC_stg_SupplierProductMapping obj in dstobj)
                        {
                            ...
                            if (objNew.CountryName == null)
                            {
                                objNew.CountryName = ((geo.Where(s => s.CityName == objNew.CityName).Select(s1 => s1.CountryName).FirstOrDefault())) ??
                                (geo.Where(s => s.CityCode == objNew.CityCode).Select(s1 => s1.CountryName).FirstOrDefault());
                            }
                            if (objNew.CountryCode == null)
                            {
                                objNew.CountryCode = ((geo.Where(s => s.CityName == objNew.CityName).Select(s1 => s1.CountryCode).FirstOrDefault())) ??
