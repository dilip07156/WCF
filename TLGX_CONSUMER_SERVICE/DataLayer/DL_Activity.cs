        ...
        public List<DataContracts.Masters.DC_ActivityProductDetailsReport> GetActivitiesProductDetailsReport(DC_ActivityCountStats _obj)
        {
            ...
            StringBuilder sborderBy = new StringBuilder();
                    ...
                    try
                    {
                        ...
                        sbSelect.Append(",InterestType = (Select STUFF((SELECT ', ' + QUOTENAME(SystemInterestType) FROM Activity_CategoriesType where Activity_Flavour_Id = spm.Activity_ID FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'),1,1,''))");
                        sbSelect.Append(",CategorySubType = (Select STUFF((SELECT ', ' + QUOTENAME(SystemProductCategorySubType)FROM Activity_CategoriesType where Activity_Flavour_Id = spm.Activity_ID FOR  XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'),1,1,''))");
                        sbSelect.Append(",ProductType = (Select STUFF((SELECT ',' + QUOTENAME(SystemProductType)FROM Activity_CategoriesType where Activity_Flavour_Id = spm.Activity_ID  FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'),1,1,''))");
                        sbSelect.Append(",ProductSubType = (Select STUFF((SELECT ',' + QUOTENAME(SystemProductNameSubType)FROM Activity_CategoriesType where Activity_Flavour_Id = spm.Activity_ID  FOR  XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'),1,1,''))");

                        sbSelect.Append(",SuitableFor = (Select STUFF((SELECT ', ' + QUOTENAME(AttributeValue) FROM Activity_ClassificationAttributes where Activity_Flavour_Id = spm.Activity_ID and AttributeType='Product'  and AttributeSubType = 'SuitableFor' FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'),1,1,''))");
                        sbSelect.Append(",PhysicalLevel = (Select STUFF((SELECT ', ' + QUOTENAME(AttributeValue) FROM Activity_ClassificationAttributes where Activity_Flavour_Id = spm.Activity_ID and  AttributeType='Product' and AttributeSubType = 'PhysicalIntensity' FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'),1,1,''))");
                        sbSelect.Append(",spm.Location As AreaAddress");

                        sbSelect.Append(",Inclusions = (SELECT CASE  WHEN EXISTS ( SELECT COUNT(1) FROM Activity_Inclusions WHERE IsInclusion = 1 and Activity_Flavour_Id = spm.Activity_ID) THEN 'YES' Else 'NO' END)");
                        sbSelect.Append(",Exclusions = (SELECT CASE WHEN EXISTS ( SELECT COUNT(1) FROM Activity_Inclusions WHERE IsInclusion = 0 and Activity_Flavour_Id = spm.Activity_ID) THEN 'YES' Else 'NO' END)");
                    ...
                    //sbSelect.Append(",ToDate = (Select CASE WHEN  Price_ValidTo is not null  THEN MAX(ISNULL(Price_ValidTo,coalesce( convert(varchar(20),Price_ValidTo) , 22 )))  else NULL END FROM Activity_Prices where Activity_Flavour_Id=spm.Activity_ID group by Activity_Flavour_Id,Price_ValidTo)");
                        ...
                        sbSelect.Append(",FromDate = (Select MIN(ISNULL(Price_ValidFrom, NULL)) FROM Activity_Prices where Activity_Flavour_Id = spm.Activity_ID)");
                        sbSelect.Append(",ToDate = (Select MAX(ISNULL(Price_ValidTo, NULL)) FROM Activity_Prices where Activity_Flavour_Id = spm.Activity_ID)");
