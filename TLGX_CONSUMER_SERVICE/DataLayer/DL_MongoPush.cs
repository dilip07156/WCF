...
namespace DataLayer
{
    public class DL_MongoPush : IDisposable
    {
        ...
        public DC_Message SyncAccommodationMaster(Guid LogId, Guid Accommodation_Id, string CreatedBy)
        {
            ...
            {
                return new DC_Message { StatusMessage = "Invalid Request.", StatusCode = ReadOnlyMessage.StatusCode.Warning }; ;
            }
        ...
        public DC_Message SyncHotelRoomTypeMapping(Guid LogId, Guid Supplier_Id)
        {
            try
            {
                ...
                using (ConsumerEntities context = new ConsumerEntities())
                {
                    ...
                    if (iScheduledCount > 0)
                    {
                        return new DC_Message { StatusMessage = "RoomType Mapping sync has already been scheduled.", StatusCode = ReadOnlyMessage.StatusCode.Information };
                    }
                }
                ...
                return new DC_Message { StatusMessage = "RoomType Mapping sync has been scheduled successfully.", StatusCode = ReadOnlyMessage.StatusCode.Success };
